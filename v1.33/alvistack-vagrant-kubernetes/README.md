...
Deploy kubernetes:
    ...
    sudo -E molecule converge -s kubernetes-1.33-libvirt -- -e 'kube_release=1.33'
    sudo -E molecule verify -s kubernetes-1.33-libvirt

All instances could be SSH and switch as root with `sudo su -`, e.g.

    cd /opt/vagrant-kubernetes
    sudo -E molecule login -s kubernetes-1.33-libvirt -h $_MOLECULE_INSTANCE_NAME-1
