...
Deploy kubernetes:
    ...
    sudo -E molecule converge -s kubernetes-1.35-libvirt -- -e 'kube_release=1.35'
    sudo -E molecule verify -s kubernetes-1.35-libvirt

All instances could be SSH and switch as root with `sudo su -`, e.g.

    cd /opt/vagrant-kubernetes
    sudo -E molecule login -s kubernetes-1.35-libvirt -h $_MOLECULE_INSTANCE_NAME-1

Check result:

    root@kube01:~# kubectl get node
    NAME     STATUS   ROLES           AGE    VERSION
    kube01   Ready    control-plane   48m   v1.35.0
    kube02   Ready    control-plane   48m   v1.35.0
    kube03   Ready    <none>          47m   v1.35.0
